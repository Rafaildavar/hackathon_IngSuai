...
import { FileText, Download, Clock, CheckCircle2, XCircle, Loader2, Upload } from "lucide-react";
      ...
      <div className="flex-1 p-6 md:p-8 lg:p-12">
        <div className="max-w-6xl mx-auto space-y-12">
          <div className="space-y-4">
            <h1 className="text-5xl md:text-6xl font-bold text-foreground">Task History</h1>
          ...
          {!isLoading && (!tasks || tasks.length === 0) && (
            <Card className="border-0 glass card-shadow">
              <CardContent className="flex flex-col items-center justify-center min-h-[400px] text-center">
                ...
                <Button asChild className="bg-primary hover:bg-primary/90 text-white" data-testid="button-upload-first">
          ...
          {tasks && tasks.length > 0 && (
            <div className="grid gap-6">
              {tasks.map((task) => (
                <Card key={task.id} className="border-0 glass card-shadow hover-elevate transition-smooth overflow-hidden">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <CardDescription>
                            {task.fileCount} {task.fileCount === 1 ? "file" : "files"} • Created {format(new Date(task.createdAt), "PPp")}
                          </CardDescription>
                        </div>
                      </div>
